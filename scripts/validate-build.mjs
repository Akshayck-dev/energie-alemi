...
if (isProdEnv) {
  if (!isAllowIndexing || !isProdUrl) {
    ...
    console.error(`Details: ALLOW_INDEXING="${ALLOW_INDEXING}" (expected "true"), SITE_URL="${SITE_URL}" (expected "https://www.energie-alemi.de")`);
    process.exit(1);
  }
}
...
if (isAllowIndexing) {
  if (!isProdEnv || !isProdUrl) {
    ...
    console.error(`Details: DEPLOYMENT_ENV="${DEPLOYMENT_ENV}" (expected "production"), SITE_URL="${SITE_URL}" (expected "https://www.energie-alemi.de")`);
    process.exit(1);
  }
}
